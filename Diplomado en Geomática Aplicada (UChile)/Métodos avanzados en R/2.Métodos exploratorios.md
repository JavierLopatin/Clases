...
Con 3 componentes ya explicamos mas del 90% de la varianza de los datos originales, los cuales 2051 bandas. Podemos graficar los primeros dos componentes y ver como los gradientes de los rasgos foliares se distribuye dentro del espacio de ordenación (ing: _feature space_):
...
Para el ejercicio 2, vamos a hacer algo similar a lo anterior pero usando los datos de especies del segundo _dataset_. Para esto vamos a utilizar la función ``metaMDS`` del paquete ``vegan`` con matrices de distancia con el índice de _Jaccard_. Recuerden que en _NMDS_ hay que decidir con anterioridad el número de componentes que se desea de acuerdo a valores bajos de _stress_. Vamos a utilizar el método del codo una vez mas. ¿Qué es lo que va a mostrar los ejes de NMDS? En este caso, cuando se hace _ordination_ con datos de vegetación los componentes muestran la composición floristica del lugar. Es decir, se ordenan los plots de acuerdo con que presencia/ausencia y cobertura del lugar. Observaciones que se encentras cerca, por lo tanto, son floristicamente parecidas, mientras que aquellas mas lejanas con floristicamente disimiles.
...
8    8           0                     65              0               0
```
 Como se puede ver, datos de composición floristica normalmente tienen muchos ceros, y distribuciones muy especiales. _NMDS_ es particularmente bueno para estos datos, junto con una distribución ad-hoc como _Jaccard_.
