...
Las metricas de regresiones buscan ver cuan parecido, cuanto error, o cuanto sesgo tienen los valores observados versus los predichos por el modelo. Conceptos básicos a tener en cuenta:

- Residuos: loa residuos son la diferencia entre los valores observados y los predichos (obs - pred). En la figura de abajo, los residuos serían las lineas rojas entre las predicciones y la linea ajustada.
- Suma de los errores cuadrados (sum of squared errors; SSE)

  $SSE = \sum_{i=1}^{n} (y_i - \bar{y})^2$, donde $y_1$
...
- RMSE (_root mean square error_): el error cuadratico medio mide el error o distancia media entre los valores predichos y los observados. Es una medida absoluta, por lo que el valor esta en la misma unidad de medida de la variable observada. Valores mas bajos son deseables.
