...
### 7.2. Métricas de ajuste o performance de los modelos
...
``caret`` permite estimar muchas de estas métricas internamente, pero también se puede utilizar el paquete ``MLmetrics``, el cual presenta muchas métricas: https://cran.r-project.org/web/packages/MLmetrics/MLmetrics.pdf

#### 7.2.1. Métricas de regresiones

Las métricas de regresiones buscan ver cuan parecido, cuanto error, o cuanto sesgo tienen los valores observados versus los predichos por el modelo. Las métricas mas comunes son:

- Error cuadratico medio (_mean square error_; MSE): Mide el error o distancia media entre los valores predichos y los observados. O entre los puntos y le recta ajustada del modelo (lineas rojas en el ejemplo de abajo). Valores mas bajos son deseables.

    $MSE= \frac{1}{n} \sum_{i=1}^{n}(y_i - \hat{y_i})^2$; donde $y_i$ son las observaciones y $\hat{y_i}$ son las predicciones.

- Raíz del error cuadratico medio (_root mean square error_; RMSE): Es la raíz cuadrada de MSE. La ventaja de RMSE sobre MSE, es que presenta valores de error en la unidad de medida de la variable observada. Por esta razon es una de las métricas más utilizadas.

    $RMSE=\sqrt{MSE}$

- Raíz del error cuadratico medio normalizada (_normalized root mean square error_; nRMSE): Es el RMSE pero normalizado por las observaciones de la variable respuesta. Esta normalización se puede hacer ustilizando la media o el rango de las observaciones. La ventaja es que el error es presentado en porcentaje (0-1 o 0-100), con lo cual es mejor para comparar modelos que utilizan distintas variables o set de datos.

    $nRMSE=\frac{RMSE}{max(y)-min(y)}$; donde $y$ son las observaciones de la variable observada.

- R²: el coeficiente de determinación (o el cuadrado de la correlación de Pearsons) es una de las métricas mas utilizadas. Si bien esta es muy útil, pueden haber ocaciones donde valores altor de R² se obtienen cuando la dispersión o error de las predicciones es alta. Valores van entre 0 y 1. Es una medida de ajuste relativa (porcentual), por lo que puede ser usada para comparar modelos entre si.

    $R²=1-\frac{n \times MSE}{\sum_{i=1}^{n}(y_i-\bar{y_i})}$; donde $n$ es el número de observaciones, $y_i$ son las variables observadas y $\bar{y_i}$ es la media de las variables observadas.

- Sesgo (_bias_): mide si hay errores sistemáticos en las predicciones del modelo. Por ejemplo, un modelo puede tener RMSE altos pero un sesgo igual a zero. Esto significaría que los errores del modelo se distribuyes aleatoreamente. Por el contrario, un modelo con bajo RMSE y sesgo positivo significaría que el modelo tiende a sobre estimar valores de la variable observada.

<p align="center">
  <img width="400" src="img/metricas.png">
</p>



#### 7.2.2. Métricas de clasificaciones

Las métricas de clasificación


### 7.3. Residulos y autocorrelación espacial

Un conceptos básicos a tener en cuenta en los modelos de regresiones son los **residuos**. Los **residuos** son la diferencia entre los valores observados y los predichos ($obs - pred$). Los **residuos** se utilizan mucho en _post-hoc tests_ (o tests para analizar los resultados de los modelos). Los **residuos**, por un lado, dan estimaciones de posibles sesgos del modelo.

Un concepto erroneo dentro de la elaboración de modelos predictivos, es que las variables observadas y predictivas deben tener una **distribución normal** para que el modelo de resultados aceptables. Esto es fundamentalmente erroneo, independientemente si el algoritmo utilizado es **paramétrico** o **no-paramétrico**. Ya que la construcción matemática de cada modelo es distinta, no vamos a revirar caso-por-caso en que ocaciones si es necesario que las variables tengan distribución normal. Sin embargo, la única regla general de todo modelo predictivo es que **los residulos de todo modelo deben en lo posible estar normalmente distribuhidos**. Sin embargo, que los **residuos** tengas una distribución normal no garantiza que el modelo no tenga sesgos. Si miramos el ejemplo de abajo, podemos ver que tanto los **residuos** de **a)** como los de **c)** tienen una distrubución normal, pero en **c)** se pueden ver claros indicios de sesgos en la gráfica de la izquierda. Esto de puede deber a variof factores: 1) el modelo puede estar subreajustado, 2) las variables estan altamente correlacionadas entre si, 3) simplemente falta información clave (co-variables) que describan la variables observada.


<p align="center">
  <img width="600" src="img/residuals.png">
</p>


Otro problema clave relacionado con modelos predictivos de regresión es la **autocorrelación espacial** (_spatial autocorrelation_). La **autocorrelación espacial** hace alusión
a que "_todas las cosas están relacionadas, pero las cosas cercanas están más relacionadas que las distantes_.". Básicamente, dos objetos cercanos tienen mayor posibilidad de estar correlacionados que objetos cercanos. Por ejemplo, la composición de especies de dos puntos separados sólo por 100 m es poblable que sea mas similar que a de dos puntos separados por 1.000 m.

Esto no es un problema en si, aunque la mayoría de los modelos predictivos asume que cada observación es independiente y no toma en cuenta la proximidad. El problema es cuando los **residuos** del modelo tienen **autocorrelación espacial**. Esto significaría que las predicciones del modelo tienen sesgos sistemáticos en el espacio, por lo que no se podría confiar en sus valores. Hay que aclarar que cierto grado de **autocorrelación espacial** en los **residuos** es normal y esperable, ya que dos valores cercanos de la varable observada van a ser similares y es normal que el modelo tienda a cometer a su vez erroros similares en ellos. Por lo que el grado de autocorrelación va a depender de la escala de trabajo.

A continuación, vamos a revisar dos métodos para estimar la **autocorrelación espacial** de los **residuos**: 1) el índice _I_ de Moran (_Moran's I index_) y 2) correlogramas.

#### 7.3.1. Moran's I

Dado un conjunto de entidades y un atributo asociado, el **índice de Moran** evalúa si el patrón expresado está agrupado, disperso o es aleatorio. La herramienta calcula el valor del Índice _I_ de Moran (i.e., similar en interpretación a un_z_-value), y un valor _P_ para evaluar la significancia de ese índice. Los valores _P_ son aproximaciones numéricas del área debajo de la curva de una distribución conocida, limitada por la estadística de prueba. Por lo tanto, los resultados se interpretan de acuerdo a _P_ e _I_:

- _P_: La hipótesis nula (H0) del test indica que **el atributo que se analiza está distribuido en forma aleatoria en el área de estudio**, por lo que en relidad lo que buscamos son valores **NO SIGNIFICATIVOS** (ej., _P_ > 0.1). Por el contrario, si el valor de _P_ es significativo, se rechaza H0 y significa que hay **autocorrelación espacial**. **OJO**, un _P_ no significativo implica en que no hay **evidencias** de autocorrelación, lo cual en términos estrictos no es lo mismo que no exista. Sin embargo, en estadística esto es sficiente en la mayor parte de los casos.
- _I_: Si el _P_ es significativo (ej., < 0.1, 0.05, o 0.001), recién el valor de _I_ toma importancia. de forma general, si _P_ no es significativo, los valores de _I_ deberían ser cercanos a cero. Normalmente es cero si el número de observaciones (_n_) es alto, o es ligeramente negativo si _n_ es bajo. Pequeños valores positivos son inusuales, pero no imposibles. Si _P_ es significativo e **_I_ es positivo**, esto quiere decir que existe una tendencia espacial a existir _clusters_ de valores, ya sean **resuduos** positivos, negativos, o ambos. Por lo tanto, no indica el tipo de sesgo, solo la existencia. Mientras que valores de **_I_ es negativos** sugiere valores alternados de **residuos**. Esto puede deberse, por ejemplo, a presencia de _outliars_ o de matrices heterogeneas de valores (un caso extremo podría ser como un tablero de ajedrez).

A continuación se ve una figura ejemplificadora de posibles valores del test de Moran y sus significados espaciales. Para explicación en detalle de índice de Moran, ver [este video](https://www.youtube.com/watch?v=_J_bmWmOF3I).

<p align="center">
  <img width="600" src="img/autocorrelacion.png">
</p>
